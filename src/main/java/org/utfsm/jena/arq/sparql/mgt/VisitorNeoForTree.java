...
public class VisitorNeoForTree implements OpVisitorTDB {
    ...
    public ReorderWeighted transform;
    public VisitorNeoForTree(ExecutionContext sContext, QueryIterator input) {
        ...
        transform = (ReorderWeighted) SystemTDB.getDefaultReorderTransform();
    }
...
//        }
    ...
    private HashMap<String, ArrayList<String>> formatTriple(Triple triple) {
        ...
        if(!tdbqueryplan.currentCardinality.containsKey(triple.getPredicate().toString())){
            tdbqueryplan.currentCardinality.put(triple.getPredicate().toString(),String.valueOf(this.transform.getTripleWeight(new PatternTriple(triple))));
        }
